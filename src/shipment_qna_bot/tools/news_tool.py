...
class NewsTool:
    ...
    def fetch_news(self, keywords: List[str], limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if self.newscatcher_api_key:
            try:
                ...
                if news:
                    logger.info(f"Successfully fetched {len(news)} articles from NewsCatcher.")
                    return news
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(news)} articles from Google News RSS.")
            return news
        except Exception as e:
    ...
    def _fetch_from_newscatcher(self, query: str, limit: int) -> List[Dict[str, Any]]:
        ...
        resp = requests.get(self.NEWSCATCHER_URL, headers=headers, params=params, timeout=10)
